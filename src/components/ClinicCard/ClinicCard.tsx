...
export function ClinicCard({ clinic, isActive, activeSpecs, onClick }: Props) {
 ...
 const hiddenCount = Math.max(0, sorted.length - MAX_SPECS);
  return (
    <div
      className={`clinic-card${isActive ? " active" : ""}`}
      onClick={onClick}
    >
      <div className="card-top">
        ...
        <div className="card-info-main">
          ...
          {clinic.address && (
            <div className="card-address">{clinic.address}</div>
          )}
        </div>
        ...
        <div className="card-specs">
          {visibleSpecs.map((s) => (
            <span
              key={s}
              className={`spec-tag${
                activeSpecs.includes(s) ? " highlighted" : ""
              }`}
            >
              {s.replace(/_/g, " ")}
            </span>
      ...
      <div className="card-likes">
        <Heart
          size={13}
          className={isActive ? "heart-filled" : "heart-empty"}
        />
        <span>
          {Math.abs(clinic.rank)} {t("ratings")}
        </span>
      </div>
    </div>
  );
